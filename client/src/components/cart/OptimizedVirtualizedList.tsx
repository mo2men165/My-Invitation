...
interface OptimizedVirtualizedListProps {
  items: CartItem[];
...
// Memoized cart item component with better performance
const CartItemComponent = memo<{
  ...
  const itemData = useMemo(() => {
    ...
    return {
      design,
      packageInfo,
      formattedDate: new Intl.DateTimeFormat('ar-SA-u-ca-gregory', {
        weekday: 'long',
        year: 'numeric',
        month: 'long',
        day: 'numeric'
      }).format(new Date(item.details.eventDate)),
      formattedStartTime: (() => {
        const [hours, minutes] = item.details.startTime.split(':');
      ...
      formattedEndTime: (() => {
        const [hours, minutes] = item.details.endTime.split(':');
        const hour = parseInt(hours);
        const period = hour >= 12 ? 'م' : 'ص';
        const displayHour = hour > 12 ? hour - 12 : hour === 0 ? 12 : hour;
        return `${displayHour}:${minutes.padStart(2, '0')} ${period}`;
  ...
  return (
    <div style={style} className="px-4 py-2">
      <div className="bg-gradient-to-br from-white/[0.02] to-white/[0.05] rounded-2xl border border-white/10 p-6 hover:border-[#C09B52]/30 transition-all duration-200 transform hover:scale-[1.01]">
        <div className="flex items-start justify-between mb-4">
          <div className="flex items-center gap-4">
            <div className="relative flex-shrink-0">
              <div className="absolute inset-0 bg-[#C09B52]/20 rounded-full blur-lg"></div>
              <itemData.packageInfo.icon className="relative w-12 h-12 text-[#C09B52]" />
            </div>
            <div className="min-w-0 flex-1">
              <h3 className="text-xl font-bold text-white truncate">{itemData.packageInfo.name}</h3>
              <p className="text-gray-400 truncate">{itemData.design?.name || 'تصميم غير معروف'}</p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2 flex-shrink-0">
            <Button
              ...
              size="icon"
              onClick={handleEdit}
              className="h-10 w-10 border-[#C09B52]/30 text-[#C09B52] hover:bg-[#C09B52]/10 transition-colors"
            >
            ...
            <Button
              ...
              className="h-10 w-10 border-red-500/30 text-red-400 hover:bg-red-500/10 transition-colors"
            >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
          ...
          <div className="flex items-center gap-2 text-gray-300">
            <Users className="w-4 h-4 text-[#C09B52] flex-shrink-0" />
            <span className="truncate">{item.details.inviteCount} دعوة</span>
          </div>
          
          <div className="flex items-center gap-2 text-gray-300">
            <Calendar className="w-4 h-4 text-[#C09B52] flex-shrink-0" />
            <span className="truncate">{itemData.formattedDate}</span>
          </div>
          
          <div className="flex items-center gap-2 text-gray-300">
            <MapPin className="w-4 h-4 text-[#C09B52] flex-shrink-0" />
            <span className="truncate">{item.details.eventLocation}</span>
          </div>
          
          <div className="text-gray-300">
            <span className="text-[#C09B52] font-semibold">
              {itemData.formattedStartTime} - {itemData.formattedEndTime}
            </span>
          </div>
        </div>
        ...
        <div className="flex items-center justify-between pt-4 border-t border-white/10">
          ...
          <div className="text-2xl font-bold text-[#C09B52] flex-shrink-0">
...
const OptimizedVirtualizedList = memo<OptimizedVirtualizedListProps>(({
  items,
  onEditItem,
  onDeleteItem,
  height = 600,
...
OptimizedVirtualizedList.displayName = 'OptimizedVirtualizedList';
