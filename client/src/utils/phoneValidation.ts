// client/src/utils/phoneValidation.ts
...
// Array of allowed country codes for react-phone-number-input
export const ALLOWED_COUNTRY_CODES = Object.keys(ALLOWED_COUNTRIES) as (keyof typeof ALLOWED_COUNTRIES)[];
...
// Function to check if a country is allowed
export const isCountryAllowed = (countryCode: string): boolean => {
  return countryCode in ALLOWED_COUNTRIES;
};

// Get error message for disallowed country
export const getDisallowedCountryError = (): string => {
  const allowedCountriesAr = Object.values(ALLOWED_COUNTRIES)
    .map(country => country.nameAr)
    .join('، ');
  
  return `الدول المسموحة فقط: ${allowedCountriesAr}`;
};
